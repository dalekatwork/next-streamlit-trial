...
import type { Optimization } from "@/types/optimizations"
    ...
    <div className="container py-8 space-y-8">
      ...
      <div className="space-y-8">
        <div className="rounded-lg border bg-card p-6">
          <h2 className="text-lg font-semibold mb-4">{labels.warehouses.chart.title}</h2>
          <UtilizationChart data={data.warehouses.utilization} />
        </div>

        <div className="rounded-lg border bg-card p-6">
          <h2 className="text-lg font-semibold mb-4">{labels.warehouses.actionables.title}</h2>
          <ActionablesTable data={data.warehouses.actionables} labels={labels.warehouses.actionables} />
        </div>

        <div className="rounded-lg border bg-card p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">{labels.warehouses.optimizations.title}</h2>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{labels.warehouses.optimizations.affected}</span>
              <span className="font-medium text-foreground">
                {data.warehouses.optimizations.visualizations} {labels.warehouses.optimizations.visualizations}
              </span>
              <span>{labels.warehouses.optimizations.and}</span>
              <span className="font-medium text-foreground">
                {data.warehouses.optimizations.pipelines} {labels.warehouses.optimizations.pipelines}
              </span>
            </div>
          </div>
          <OptimizationsTable 
            data={data.warehouses.optimizations.items as Optimization[]} 
